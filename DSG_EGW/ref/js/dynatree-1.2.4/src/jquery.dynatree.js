...
/* modified by HANDY Directory : START */
			} else if (href == "#") {
				nodeTitle = '<a style="cursor:pointer;" class="' + opts.classNames.title + '"' + tooltip + '>' + data.title + '</a>';
/* modified by HANDY Directory : END */
			} else {
...
/* modified by HANDY Directory 2.0.1 : START */
/* dtnode.data.unselectable 값에 따라 checkbox/radio css 변경하도록 수정 */
		if ( data.unselectable ) {
			cnList.push( this.bSelected ? "ui-dynatree-unselectable-selected" : "ui-dynatree-unselectable" );
		} else {
			if( this.bSelected ){
				cnList.push(cn.selected);
			}
		}
/* modified by HANDY Directory 2.0.1 : END */
			if( this.hasSubSel ){
...
/* modified by HANDY Directory 2.0.1 : START */
/* single select mode에서 unselectable노드가 checked일 경우, 다른 노드의 radio버튼이 클릭되면 unselectable노드를 unchecked로 변경 */
		if ( this.data.unselectable ) {
			$(this.span).removeClass("ui-dynatree-unselectable-selected");
			$(this.span).addClass("ui-dynatree-unselectable");
		} else {
...
/* modified by HANDY Directory 2.0.1 : START */
/* activate 노드의 상위 노드를 접었다 펼치면, activate 상태가 해제되는 문제 수정 */
			// this.tree.activeNode.deactivate();
/* modified by HANDY Directory 2.0.1 : END */
		}
...
*/
	...
	bind: function() {
		...
		this.element.bind(eventNames, function(event){
			...
			// modify by Square Plus.
			// Prevent event when title is empty.
			if(dtnode.data.title == "") {
				return true;
			}
